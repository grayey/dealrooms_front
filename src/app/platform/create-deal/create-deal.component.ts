...
export class CreateDealComponent extends MagicClasses implements OnInit, AfterViewInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.createDealForm = this.fb.group(CreateDealComponent.createDealForm());
    this.updateDealForm = this.fb.group(CreateDealComponent.updateDealForm());
    ...
    await  this.storage.upload(path, this.selectedDocumentFile).snapshotChanges().pipe(
      finalize(() => {
        fileRef.getDownloadURL().subscribe((url) => {
          ...
          const documentObject = {temp_id, name, url, type, icon, created_at: date.toString()};
  ...
  public downloadDocument(doc) {
    ...
    setTimeout(() => {
      doc['fetching'] = false;
      this.downloadFirebaseFile(doc);
    }, 2000);

    // const docUrl = this.storage.ref(doc['url']).getDownloadURL().subscribe((res)=>{
    //   console.log(res,'...')
    // });
  ...
  public setCreateIntent(){
    Cache.set('WANTED_TO_CREATE',true);
  }
